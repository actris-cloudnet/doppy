...
from doppy.product import Stare, Wind
...
def from_stare_and_wind(
    ...
    integration_time : float, seconds
        Time used for individual stare measurement
    ...
    length_scale_individual = _compute_length_scale_individual(
        horizontal_wind_speed, stare.radial_distance, integration_time, beam_divergence
    )
    ...
    with np.errstate(invalid="ignore"):
        dissipation_rate = (
            ...
            * var ** (3 / 2)
            * (length_scale_sampling ** (2 / 3) - length_scale_individual ** (2 / 3))
            ** (-3 / 2)
        )
    return np.array(dissipation_rate, dtype=np.float64)


def _compute_variance(
    stare: Stare, time_period: int = 1800
    ...
    df_var = df.with_columns(
        pl.exclude("dt").rolling_var_by(
            "dt", window_size=time_period_str, closed="both", ddof=0
        )
    )
    ...
    return np.array(var, dtype=np.float64), np.array(
        sampling_time[:, np.newaxis], dtype=np.float64
    )
